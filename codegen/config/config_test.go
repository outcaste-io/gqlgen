...
func TestConfigCheck(t *testing.T) {
	t.Run("invalid config format due to conflicting package names", func(t *testing.T) {
		...
		require.EqualError(t, config.check(), "exec and model define the same import path (github.com/outcaste-io/gqlgen/codegen/config/generated) with different package names (graphql vs generated)")
	})
	...
	t.Run("federation must have same package name as exec", func(t *testing.T) {
		...
		require.EqualError(t, config.check(), "exec and federation define the same import path (github.com/outcaste-io/gqlgen/codegen/config/generated) with different package names (generated vs federation)")
	})
...
func TestAutobinding(t *testing.T) {
	t.Run("valid paths", func(t *testing.T) {
		cfg := Config{
			Models: TypeMap{},
			AutoBind: []string{
				"github.com/outcaste-io/gqlgen/example/chat",
				"github.com/outcaste-io/gqlgen/example/scalars/model",
			},
		...
		require.Equal(t, "github.com/outcaste-io/gqlgen/example/scalars/model.Banned", cfg.Models["Banned"].Model[0])
		require.Equal(t, "github.com/outcaste-io/gqlgen/example/chat.Message", cfg.Models["Message"].Model[0])
	})
